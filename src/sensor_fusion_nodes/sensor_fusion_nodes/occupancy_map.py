...
class OccupancyMap(Node):
    ...
    def object_callback(self, msg: MarkerArray):
        ...
        self.profiler.record_many({
            'map_copy': (t_copy - t_total) * 1000.0,
            'map_painting': (t_paint - t_copy) * 1000.0,
            'semantic_map_publish': (t_end - t_paint) * 1000.0,
            'total_occupancy_map_callback': (t_end - t_total) * 1000.0,
        }, num_confirmed_objects=len(msg.markers))
