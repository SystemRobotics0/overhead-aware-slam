...
from sensor_fusion_nodes.runtime_profile import RuntimeProfiler
...
class ObjectLocation(Node):
    ...
    def __init__(self):
        ...
        self.init_params()
        self.declare_parameter('runtime_profile_csv', '')
        self.declare_parameter('runtime_profile_flush_every', 50)
        self.profiler = RuntimeProfiler(
            'object_location',
            self.get_parameter('runtime_profile_csv').value,
            self.get_parameter('runtime_profile_flush_every').value,
        )
        self.init_memory()
        self.init_ros()
    ...
    def try_process(self):
        ...
        t_lidar_extract = time.perf_counter()
        ...
        t_clustering = time.perf_counter()
        ...
        t_cluster_select = time.perf_counter()
        ...
        t_type_context = time.perf_counter()
        ...
        t_publish_geometry = time.perf_counter()
        ...
        self.profiler.record_many({
            'scan_lookup': (t_scan_sync - t_total) * 1000.0,
            'wedge_projection': (t_wedge - t_scan_sync) * 1000.0,
            'wedge_filtering': (t_lidar_extract - t_wedge) * 1000.0,
            'clustering': (t_clustering - t_lidar_extract) * 1000.0,
            'cluster_selection': (t_cluster_select - t_clustering) * 1000.0,
            'support_geometry': (t_geometry - t_cluster_select) * 1000.0,
            'tf_transform': (t_tf - t_geometry) * 1000.0,
            'structural_filter': (t_structural - t_tf) * 1000.0,
            'fill_ratio_and_classification': (t_type_context - t_structural) * 1000.0,
            'object_memory_update': (t_memory - t_type_context) * 1000.0,
            'geometry_publish': (t_publish_geometry - t_memory) * 1000.0,
            'wedge_publish': (t_end - t_publish_geometry) * 1000.0,
            'total_fusion_callback': (t_end - t_total) * 1000.0,
        },
            num_detections=1,
            num_lidar_points=len(raw_pts),
            num_clusters=len(clusters),
            num_confirmed_objects=len(self.confirmed_objects),
        )
